...
public class VodCall {
	...
	int Init(String secId, String secKey, int iUsage, int threadNum) {
		...
		m_iUgcRandom = new Random().nextInt(Integer.MAX_VALUE);
	...
	int InitCommPara(TreeMap<String, Object> mapVals) {
		...
		mapVals.put("Nonce", new Random().nextInt(Integer.MAX_VALUE));
		return 0;
	}
...
//			timerLog.info("GetReqSign >> " + contextStr);
//			getSign(s, m_strSecKey);
	...
	String GetUgcExSign() {
		...
		try {
		//这些参数，由客户端带到svr
		contextStr += "f=" + URLEncoder.encode(m_strFileName, "utf8");
		...
		contextStr += "&s=" + URLEncoder.encode(this.m_strSecId, "utf8");
		contextStr += "&uid=" + "1";
	...
	public String GetReqUrl(TreeMap<String, Object> mapVals, String strSign) throws UnsupportedEncodingException {
		...
		for (String key : mapVals.keySet()) {
			...
			reqStr += key + '=' + URLEncoder.encode(mapVals.get(key).toString(), "utf8").replace("*", "%2A").replace("+", "%20").replace("%7E", "~");
		}
...
//				timerLog.info("%d %d", ret, partInfo.m_dataSize);
				return -2001;
			}
...
//				timerLog.info("%d %d", ret, partInfo.m_dataSize);
			...
			String strSign = GetReqSign(mapVals);
			String strReq = GetReqUrl(mapVals, strSign);
			timerLog.info("multipartUpload 分片上传片段请求 >> "+strReq);
			JSONObject jsonobj= null;
			try {
				jsonobj = DoHttpReq(strReq, buf, (int) (partInfo.m_dataSize));
                timerLog.info("multipartUpload 分片上传片段上传结果 >> "+jsonobj);
			} catch (Exception e) {
				if (retryLimit > 0) {
					retryLimit--;
					continue;
	...
	public int InitUpload() throws IOException, JSONException {
		...
		while (true) {
			...
			String strReq = GetReqUrl(mapVals, strSign);
			timerLog.info("multipartUpload 分片上传初始化请求 >> "+strReq);
			JSONObject jsonobj= null;
			try {
				jsonobj = DoHttpReq(strReq, null, 0);
			} catch (Exception e) {
	...
	public int Report(int code) throws IOException, JSONException {
		...
		JSONObject jsonobj= null;
		jsonobj = DoHttpReq(strReq, null, 0);
//		timerLog.info("speed : "+m_qwFileSize/(m_qwEndTime - m_qwStartTime) + "byte/s");
		return 0;
	}
	...
	public int Upload() {
		try {
			int iRet = InitUpload();
			if (iRet != 0) {
                timerLog.info("multipartUpload 分片上传初始化失败 >> " + iRet);
				//Report(iRet);
				return iRet;
			}
            timerLog.info("multipartUpload 分片上传初始化成功 >> "+iRet);
			...
			for (int i = 0; i < m_arrThreadList.size(); i++) {
				if (m_arrThreadList.get(i).m_iRet != 0) {
                    timerLog.info("multipartUpload 分片上传部分片段上传失败 >> " + m_arrThreadList.get(i).m_iRet +"\n");
			...
			if (iRet != 0) {
                Report(iRet);
                timerLog.info("multipartUpload 分片上传合并失败 >> "+ iRet);
				return iRet;
			}
			this.m_qwEndTime = System.currentTimeMillis() / 1000;
			//Report(0);
            timerLog.info("multipartUpload 分片上传上传成功 >> "+ iRet);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return 0;
