...
public class Request {
	...
	protected static int timeOut = 30*1000;//设置连接主机的超时时间，单位：毫秒，可以根据实际需求合理更改 timeOut 的值。
	protected static int readTimeOut = 30*1000;//设置读取超时 add by kpc 解决回放假死问题
	...
	public static String sendRequest(String url,
		...
		try {
			...
			result = "{\"code\":-2700,\"location\":\"com.qcloud.Common.Request:225\",\"message\":\"api sdk throw exception! "
					+ e.toString() + "\"}";
            httpQcloudLog.error(result, StackTraceUtils.deepSanitize(e));
		} finally {
			// 使用finally块来关闭输入流
			try {
				if (in != null) {
				...
				result = "{\"code\":-2800,\"location\":\"com.qcloud.Common.Request:234\",\"message\":\"api sdk throw exception! "
						+ e2.toString() + "\"}";
                httpQcloudLog.error(result,  StackTraceUtils.deepSanitize(e2));
			}
		}
		rawResponse = result;
		return result;
	...
	public static String sendMultipartUploadVodFileRequest(String url,
		...
		try {
            ...
            //-------------------------------自定义上传,超过2G文件导致 start-------------------------------------------
            RandomAccessFile ins = new RandomAccessFile(file,"r");
            long offset = ((Long) requestParams.get("offset")).longValue();
            long dataSize = ((Long) requestParams.get("dataSize")).longValue();
            ...
            ins.seek(offset);
            long page = dataSize / 1024;
            long remainder = dataSize % 1024;
            int remainderInt = (int)remainder;
            int bytes = 0;
            byte[] bufferOut = new byte[1024];
            byte[] bufferOut2 = new byte[remainderInt];
            while (page != 0) {
