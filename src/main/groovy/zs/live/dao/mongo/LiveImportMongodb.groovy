...
@Slf4j
@CompileStatic
class LiveImportMongodb {

    @Value('${live.import.mongo.servers}')
    String servers // host:port,host:port,host:port,...
    @Value('${live.import.mongo.db}')
    String dbName
    @Value('${live.import.mongo.connectionsPerHost}')
    int  connectionsPerHost
    @Value('${live.import.mongo.connectionMultiplier}')
    ...
    @CompileStatic
    public List findLiveRecordList(String collName, DBObject query, DBObject sort, int psize){
        DBCursor cursor = null;
        List ret = []
        try {
            DBCollection coll = mongoDB.getCollection(collName)
            cursor = coll.find(query).sort(sort).limit(psize)
            ret = cursor?.toArray()
        }catch (Exception e){
            e.printStackTrace()
        }finally {
